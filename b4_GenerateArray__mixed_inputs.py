...
def get_label(image, folder_name):
    ...
    raw_img_path = os.path.join(san, 'images_resized', folder_name, name)
    # print('raw_img_path', raw_img_path)
    if os.path.exists(raw_img_path):
        # print('Read data from ', raw_img_path)
        image = cv2.imread(raw_img_path, 1)
        image = image[image.shape[0]-HEIGHT:, :]
        # print(image.shape)
        # image = cv2.resize(image, (WIDTH, HEIGHT))
        # cv2.imshow('image', image)
        # cv2.imshow('img', img)
        # cv2.waitKey(1)
        # return cv2.resize(image,(240, 160)), center[0]*1.0/WIDTH, correct
    ...
    for imgname in os.listdir(fpath):
        ...
        if correct is not None:
            if correct:
                ...
                X_img.append(image)
                X_route.append(routes_mapping[route])
                X_lane.append(lanes_mapping[lanepath])
                Y.append([center])
...
X_lane = np.array(X_lane)
Y = np.array(Y)
np.save(data_path_X_img, X_img)
np.save(data_path_X_route, X_route)
np.save(data_path_X_lane, X_lane)
